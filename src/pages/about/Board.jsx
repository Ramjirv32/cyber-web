...
const boardMembers = [
  {
    ...
    organization: "Mélange Academic Research Associates",
        ...
        >
          <div className="h-64 bg-gray-200 relative overflow-hidden">
            ...
            <div className="absolute bottom-4 left-4 text-white">
              ...
              {member.credentials && <p className="text-sm opacity-90">{member.credentials}</p>}
            </div>
          </div>

          <div className="p-6 space-y-4">
            {member.title && <h4 className="font-bold text-red-500">{member.title}</h4>}
            {member.organization && (
                ...
                <div>
                  {Array.isArray(member.email) ? (
                    member.email.map((email) => (
                      <a key={email} href={`mailto:${email}`} className="text-red-500 hover:text-red-600 block">
                        {email}
                      </a>
                    ))
                  ) : (
                    <a href={`mailto:${member.email}`} className="text-red-500 hover:text-red-600">
                      {member.email}
                    </a>
                  )}
                </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  {typeof member.phone === "object" ? (
                    <>
                      <p className="text-gray-600">SK: {member.phone.sk}</p>
                      <p className="text-gray-600">IND: {member.phone.ind}</p>
                    </>
                  ) : (
